...
def test_music_catalog_groups_and_orders_albums_and_standalone_songs():
    ...
    Song.objects.create(title="Second", slug="second", album=late_album, track_number=2)
    Song.objects.create(title="First", slug="first", album=late_album, track_number=1)
    ...
    assert [song["track_number"] for song in payload["albums"][1]["songs"]] == [1, 2]
    ...
    assert all(
        set(song) == {"id", "title", "slug", "track_number", "is_b_side"}
